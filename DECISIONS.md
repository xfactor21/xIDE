...
## 34. Isolated Conversation Intelligence Boundary
**Decision**: Implemented an isolated, secure developer assistance layer (via `XeroConversationEngine` and `ContextSelector`) that handles developer inquiries, classifies intent, and gathers localized context under bounded in-memory structures without mutating files or executing system-level actions. All action proposals generated by this layer are strictly submitted to the explicit `ActionApprovalManager` without bypassing approval validation barriers.
**Reason**: This completely limits Xero's conversational assistant to a non-intrusive, read-only analytical mode. By forcing all generated files or code proposals to be registered in the `ActionApprovalManager` first, the system guarantees that no file creation or modification is ever executed automatically or implicitly through conversational chat. Furthermore, token and secret guards in the `ContextSelector` ensure sensitive data is never leaked or sent to any LLM engine.

## 35. Unified IDE Navigation & Modular Decoupled Panels
**Decision**: Designed `WorkspaceNavigationManager` using a reactive `StateFlow` to manage tab selections, active document tabs, and editor focus, keeping individual panels completely decoupled from each other.
**Reason**: Prevents spaghetti state dependencies. By using a single, unified navigation manager, panels like `ProjectExplorerPanel`, `EditorWorkspacePanel`, and `DiagnosticsPanel` can easily trigger navigation flows (e.g., clicking on a file in the explorer or an error line in diagnostics instantly navigates to and opens that file in the editor) without direct cross-references or architectural coupling.

## 36. Chat-Integrated Action Proposal Cards
**Decision**: Embedded interactive approval cards directly within the chat message stream of the `XeroAssistantPanel`.
**Reason**: Enhances usability and enforces safe human-in-the-loop workflows. Rather than navigating to a separate dashboard, the developer can view Xero's proposed file mutations in context and immediately approve or reject them, leading to a fluid and safe development journey.

## 37. Client-Side Read-Only Workspace Representation (MANDATORY)
**Decision**: Standardized that UI panels interact with underlying systems solely via defined service boundaries (`VirtualFileSystem`, `BuildService`, `DiagnosticsEngine`, `XeroConversationEngine`), remaining as pure rendering layers with absolutely zero independent execution or bypass authorities.
**Reason**: Strict containment and security. By preventing UI panels from directly writing to the physical filesystem, executing commands, or invoking raw AI APIs, we guarantee that all operations flow through the robust validation, sanitization, and approval checks implemented in the core platform layers.
...
## 38. Lightweight Semantic Graph before AST
**Decision**: Adopted a lightweight semantic graph layer built upon regex symbol extraction within `ProjectIndexerImpl`, deliberately deferring a full compiler-grade AST parser (like KSP or Kotlin Compiler API).
**Reason**: To maintain fast index processing times and strict offline operability, while sufficiently satisfying advanced "find usages", "dependency analysis", and context generation queries necessary for Phase 21 intelligence operations.
## 39. Lightweight Semantic Editor Intelligence
**Decision**: Standardized on a lightweight regex-driven semantic extraction engine (`EditorAnalysisEngine`, `SemanticHighlightProvider`) before committing to a full compiler-grade integration (like LSP or KSP-based syntax trees).
**Reason**: Editor analysis needs to be fast, asynchronous, and non-blocking on every keystroke. Full AST reconstruction is too heavy for the current bounded execution environment, while lightweight regex parsing fulfills the Phase 22 requirements of syntax highlighting, unused import detection, and improvement suggestions.
